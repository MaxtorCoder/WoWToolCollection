...
namespace CASCLib
{
    ...
    public class WowRootHandler : RootHandlerBase
    {
        ...
        public override void LoadListFile(string path)
        {
            ...
            if (!File.Exists(path))
            {
                Console.WriteLine("WowRootHandler: list file missing!");
                return;
            }

            bool isCsv = Path.GetExtension(path) == ".csv";

            Console.WriteLine($"WowRootHandler: loading listfile {path}...");
        ...
        protected override CASCFolder CreateStorageTree()
        {
            ...
            Console.WriteLine("WowRootHandler: {0} file names missing for locale {1}", CountUnknown, Locale);
        ...
        {
            Console.WriteLine("WowRootHandler Dump:");
            ...
            foreach (var fd in RootData.OrderBy(r => r.Key))
            {
                ...
                Console.WriteLine("{0:D7} {1:X16} {2} {3}", fd.Key, fd.Key, fd.Value.Aggregate(LocaleFlags.None, (a, b) => a | b.LocaleFlags), name);

                foreach (var entry in fd.Value)
                {
                    Console.WriteLine("\t{0} - {1} - {2}", entry.MD5.ToHexString(), entry.LocaleFlags, entry.ContentFlags);
                }
            }
        }
    }
