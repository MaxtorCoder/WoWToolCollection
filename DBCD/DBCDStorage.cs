...
namespace DBCD
{
    ...
    public interface IDBCDStorage : IEnumerable<DynamicKeyValuePair<int>>, IDictionary<int, DBCDRow>
    {
        ...
        Y GetField<Y>(int id, string columnName);
    }
    ...
    public class DBCDStorage<T> : ReadOnlyDictionary<int, DBCDRow>, IDBCDStorage where T : class, new()
    {
        ...
        public Y GetField<Y>(int id, string columnName)
        {
            if (!Dictionary.TryGetValue(id, out DBCDRow row))
                return default;

            var stringLen = columnName.Length;
            if (columnName.Contains("[") && columnName.Contains("]"))
            {
                var indexOfOpen = columnName.IndexOf('[');
                var indexOfClose = columnName.IndexOf(']');
                stringLen = columnName.Length - (indexOfClose - indexOfOpen) - 1;
            }

            var normalizedColumnName = columnName.Substring(0, stringLen);
            if (!info.availableColumns.Contains(normalizedColumnName))
                return default;

            // requesting array type
            if (columnName.Contains("[") && columnName.Contains("]"))
            {
                var rowData = row[normalizedColumnName];
                if (rowData.GetType().IsArray)
                {
                    var arrayIdx = int.Parse(columnName.Replace(normalizedColumnName, "")
                                                       .Replace("[", "")
                                                       .Replace("]", ""));

                    var rowArrayData = (Array)rowData;

                    return (Y)Convert.ChangeType(rowArrayData.GetValue(arrayIdx), typeof(Y));
                }
            }
            else if (row[columnName].GetType().IsArray)
                return (Y)Convert.ChangeType((Array)row[columnName], typeof(Y));

            return (Y)Convert.ChangeType(row[columnName], typeof(Y));
        }
